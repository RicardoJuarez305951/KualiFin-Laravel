<div>
    ...
    <div class="mt-6 flex justify-between">
        @if($step > 0)
            <button wire:click="previousStep" class="px-4 py-2 bg-gray-200 rounded-lg">
                ← Anterior
            </button>
        @endif

        @if($step < 5)
            <button wire:click="nextStep" class="px-4 py-2 bg-blue-600 text-white rounded-lg">
                Continuar →
            </button>
        @else
            <button wire:click="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg">
                Enviar Solicitud
            </button>
        @endif
    </div>
