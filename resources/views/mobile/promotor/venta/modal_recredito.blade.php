    ...
    <form x-ref="formRecredito" method="POST" action="{{ route('mobile.promotor.store_recredito') }}" @submit.prevent="submitRecredito" class="space-y-4">
      ...
      <div class="mb-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <button type="button" @click="r_newAval = false" :class="r_newAval ? 'border border-blue-800 text-blue-800 bg-white' : 'bg-blue-800 text-white'" class="w-full rounded-xl px-3 py-2 font-semibold transition">
            Mismo Aval
          </button>
        ...
        <p class="mt-2 text-xs text-gray-600" x-text="r_newAval ? 'Captura un nuevo aval para este recrédito.' : 'Se usará el aval previo registrado.'"></p>
      </div>
